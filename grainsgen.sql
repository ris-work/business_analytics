...
INSERT INTO hourly SELECT * FROM (WITH randomdates AS (SELECT x AS date FROM dates WHERE x BETWEEN datetime('now', '-730 days') AND datetime('now')), madeup AS (SELECT itemcode, randomdates.date, 10+ABS(RANDOM())%10 AS timehour, ABS(RANDOM())%100 AS qty, cost, sell FROM randomdates, products WHERE ABS(RANDOM()%10)=1) SELECT itemcode, date as daydate, timehour, qty AS quantity, sell*qty AS sumsell, cost*qty AS sumcost FROM madeup);
