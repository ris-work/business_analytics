...
$stmt_sql = $dbh->prepare("SELECT date, closestfuture.sih AS SIH, total(dailyqty) OVER (ORDER BY date ROWS BETWEEN 14 PRECEDING AND CURRENT ROW) AS s15, total(dailyqty) OVER (ORDER BY date ROWS BETWEEN 29 PRECEDING AND CURRENT ROW) AS s30, total(dailyqty) OVER (ORDER BY date ROWS BETWEEN 59 PRECEDING AND CURRENT ROW) AS s60 FROM (SELECT total(quantity) as dailyqty, x as date, itemcode FROM dates LEFT JOIN (SELECT daydate, total(quantity) AS quantity, itemcode AS itemcode FROM hourly WHERE itemcode=? GROUP BY daydate) daily ON dates.x = daily.daydate WHERE x > (SELECT min(daydate) FROM hourly WHERE itemcode=?) GROUP BY x ORDER BY x) frommindate LEFT JOIN (SELECT * FROM (SELECT datetime, itemcode, sih FROM sih_history UNION SELECT datetime('now'), CAST(itemcode AS int) AS itemcode, sih FROM sih_current) WHERE itemcode=? ORDER BY datetime LIMIT 1) closestfuture ON date < closestfuture.datetime");
$stmt = $stmt_sql->execute([$response->PLU_CODE, $response->PLU_CODE, $response->PLU_CODE]);
