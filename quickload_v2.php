...
$stmt_sql = $dbh->prepare("SELECT sih_current.itemcode AS PLU_CODE, desc AS PLU_DESC, sih_current.sell/iif(sih=0,1,sih) AS PLU_SELL_AV, (iif(iif(latestsell.asell > 0, latestsell.asell, selling.sell) > 0, iif(latestsell.asell > 0, latestsell.asell, selling.sell), full_inventory_current.sell)) AS PLU_SELL, sih_current.sih AS SIH, coalesce(iif(cost_purchase.cost > 0, cost_purchase.cost, latestsell.acost), 'UNKNOWN') AS cost FROM sih_current INDEXED BY sih_cast_covering LEFT JOIN selling INDEXED BY selling_covering ON selling.itemcode = sih_current.itemcode LEFT JOIN cost_purchase ON cost_purchase.itemcode = sih_current.itemcode LEFT JOIN (SELECT min(asell) AS asell, max(acost) AS acost, maxdates.itemcode AS itemcode FROM ((SELECT cast(itemcode AS int) as itemcode, daydate, sumsell/quantity as asell, sumcost/quantity as acost FROM hourly INDEXED BY hourly_sales_averages) sales JOIN (SELECT max(daydate) AS maxdate, cast(itemcode AS int) AS itemcode FROM hourly INDEXED BY maxdates_int GROUP BY cast(itemcode AS int)) maxdates ON maxdates.itemcode = sales.itemcode AND maxdates.maxdate = sales.daydate) GROUP BY sales.itemcode) latestsell ON latestsell.itemcode = cast(sih_current.itemcode AS int) LEFT JOIN full_inventory_current INDEXED BY full_inventory_current_covering ON cast(sih_current.itemcode AS int) = full_inventory_current.itemcode");
...
$stmta_sql = $dbh->prepare("SELECT ifnull(S_D60, 0) AS S_D60, ifnull(S_D30, 0) AS S_D30, ifnull(S_D15, 0) AS S_D15, everything.itemcode AS CODE FROM sih_current AS everything INDEXED BY sih_cast_covering_beginning_with_uncast LEFT JOIN (SELECT total(quantity) as S_D60, cast(itemcode AS int) as itemcode FROM hourly INDEXED BY hourly_index_for_trends_cast_with_uncast WHERE daydate BETWEEN date((SELECT max(daydate) FROM hourly), '-61 days') AND date((SELECT max(daydate) FROM hourly), '-1 day') GROUP BY cast(itemcode AS int)) S60T ON cast(everything.itemcode AS int) = cast(S60T.itemcode AS int) LEFT JOIN (SELECT total(quantity) AS S_D30, cast(itemcode AS int) AS itemcode FROM hourly INDEXED BY hourly_index_for_trends_cast_with_uncast WHERE daydate BETWEEN date((SELECT max(daydate) FROM hourly), '-31 days') AND date((SELECT max(daydate) FROM hourly), '-1 day') GROUP BY cast(itemcode AS int)) S30T ON S30T.itemcode = S60T.itemcode LEFT JOIN (SELECT total(quantity) AS S_D15, cast(itemcode AS int) AS itemcode FROM hourly INDEXED BY hourly_index_for_trends_cast_with_uncast WHERE daydate BETWEEN date((SELECT max(daydate) FROM hourly), '-16 days') AND date((SELECT max(daydate) FROM hourly), '-1 day') GROUP BY cast(itemcode AS int)) AS S15T ON S15T.itemcode = S30T.itemcode ");
