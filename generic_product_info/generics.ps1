...
Get-Date -Format "o"
Invoke-Sqlcmd -ServerInstance "$serv" -Query "$query" -Encrypt "Optional" -TrustServerCertificate -User "pos" -Password "$cred" | ConvertTo-csv -NoHeader | Out-File -File generics_product_info.csv.inprogress && mv generics_product_info.csv.inprogress generics_product_info.csv
