...
async function displaySelected(){
	...
	if(input.length>=3){
		...
		if(regex){
			...
			if(starts_with){
				json_filtered = json_pl.filter(a => a.label_i18n_en.startsWith(il));
			}
			else if(abjad){
				...
				ilnseg = iln.split(" ");;
				ilnseg.forEach((v)=>json_filtered = json_filtered.filter(a => a.label_i18n_en.includes(v)));
			}else{
				json_filtered = json_pl.filter(a => a.label_i18n_en.includes(il));
			}
			not_in_cache = json_filtered.filter(a => {return false; !DataCache.has(a.PLU_CODE) || !AnalyticsCache.has(a.PLU_CODE)});
			...
			in_cache.forEach(x => data.push(DataCache.get(x.itemcode)));
			...
			for(var i1 in data){
				...
				try{
					...
					Object.assign(dwa, DataCache.get(data[i1].itemcode));
...
function pretty_print_filtered(filtered){
	...
	heading_row.appendChild(generate_data_heading("Tamil"));
	heading_row.appendChild(generate_data_heading("Sinhala"));
...
function loaded(){
	document.getElementById("back").addEventListener("click", goBack)
		...
		json_p=list_p;
	json_pa=json_p.map(v => {var copy = Object.assign({}, v); copy.label_i18n_en = normalize(copy.label_i18n_en + copy.label_i18n_ta + copy.label_i18n_si); return copy});
	json_pl=json_p.map(v => {var copy = Object.assign({}, v); copy.label_i18n_en = (copy.label_i18n_en.toLowerCase()); return copy});
...
$stmt_sql = $dbh->prepare(
		"
SELECT l.itemcode, desc AS label_i18n_en, label_i18n_ta, label_i18n_si FROM label_i18n l JOIN sih_current s ON s.itemcode = l.itemcode;
"
);
...
<script>
list_p.forEach((v) => {DataCache.set(v.itemcode, v)});
