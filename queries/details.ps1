...
Invoke-Sqlcmd -ServerInstance "$serv" -Query "SELECT count(*), `'$name`' FROM $name" -Encrypt "Optional" -TrustServerCertificate -User "pos" -Password "$cred" | ConvertTo-csv -NoHeader | Out-File -Append -File size.csv.inprogress 
Get-Date -Format "o"
}
