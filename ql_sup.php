...
function generate_table_row(v){
	...
	if(v){
		...
		if(parseInt(v.SIH) <parseInt((v.S_D15)*7/60))row.className="very-dangerous";
...
INNER JOIN sih_current ON product_vendors.itemcode = sih_current.itemcode
...
$stmt = $stmt_sql->execute([$_GET["vendorcode"]]);
$cur_data = $stmt_sql->fetchAll();
$dbh->commit();
$response = $cur_data;
$response_dec = $response; // json_decode($response);
$t_q1_end = microtime(true);
$t_supname = $dbh->beginTransaction();
$stmt_sql_supname=$dbh->prepare("SELECT vendorname FROM vendors WHERE vendorcode=?");
$stmt_supname = $stmt_sql_supname->execute([$_GET["vendorcode"]]);
$cur_data_supname = $stmt_sql_supname->fetchAll();
$dbh->commit();
$response = $cur_data;
$response_dec = $response; // json_decode($response);
$t_q1_end = microtime(true);
...
$stmta = $stmta_sql->execute([$_GET["vendorcode"]]);
...
//var_dump($response);
?>
	<title>DETAILS: <?php echo $lastu[0][0] . " " . $cur_data_supname[0]["vendorname"]; ?></title>
<script>
...
<span class="notice">Displaying data ONLY for: <?php echo $cur_data_supname[0][0]; ?></span> <br />
