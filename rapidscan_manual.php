...
function lookupAndAdd(barcode){
		...
		if(barcode){
			var looked_up_from_existing_or_current = json_pa.filter(a => a.PLU_DESC.includes(normalize(barcode)));
			if(looked_up_from_existing_or_current.length > 10){
					alert(`Too many matches, matched ${looked_up_from_existing_or_current.length}`);
					return;
			}
...
function pretty_print_filtered(filtered){
	...
	var clear_all_button_th = document.createElement("th");
	var clear_all_button = document.createElement("button");
	clear_all_button.value = "X";
	clear_all_button.verticalAlign = "Center";
	clear_all_button.innerText = "\uD83D\uDDD1";
	clear_all_button.addEventListener("click", () => { json_filtered=[]; lookupAndAdd() });
	clear_all_button_th.appendChild(clear_all_button);
	heading_row.appendChild(clear_all_button_th);
...
function addFromTextBox(){
		lookupAndAdd(document.getElementById("manual_entry").value);
}
function loaded(){
	document.getElementById("back").addEventListener("click", goBack)
	document.getElementById("add_manual").addEventListener("click", addFromTextBox)
		...
		json_p=list_p;
	json_pa=json_p.map(v => {var copy = Object.assign({}, v); copy.PLU_DESC = normalize(copy.possible_barcodes + " " + copy.PLU_DESC); return copy});
...
<span class="notice">Data loaded on (please check today's date): <?php echo $lastu[0][0]; ?></span> <br />
		<div id="everything">
		<div class="centered-container">
			<label style="font-size: 2em; font-family: MONOSPACE" for="manual_entry">
				Search and add for: 
			</label>
			<input style="font-size: 2em; font-family: MONOSPACE" type="text" id="manual_entry" placeholder="🔍" />
			<input type="submit" id="add_manual"/>
		</div><br />
